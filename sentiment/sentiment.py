...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, lexicon_file="lexicon.txt", emoji_lexicon="emoji_utf8_lexicon.txt"):
...
if __name__ == '__main__':
 ...
 ds=pd.read_csv('data/input.csv',encoding='cp1252')
 print(ds.head(5))
 
 df_res = pd.DataFrame(columns=['input', 'label'])
 for index, row in ds.iterrows():
    res = analyzer.polarity_scores(str(row[0]))
    #print(res)
    final_res=''
    if (res['compound']) >= 0.1:
        final_res="Positive"
        ...
        final_res="Netural"
    df_res = df_res.append({'input': str(row[0]), 'label': final_res}, ignore_index=True)
    
df_res.to_csv(r'data/output.csv', index = False)
